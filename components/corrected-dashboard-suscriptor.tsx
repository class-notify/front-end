...
import { StatusBadge, statusColors } from "@/components/ui/improved-status-badge"
import { CalendarView } from "./calendar-view"
import { ClaseDetailModal } from "./clase-detail-modal"
import { Calendar, Clock, MapPin, Bell, Settings, User, BookOpen, Users, Grid3X3, Filter } from "lucide-react"
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select"
import type { Clase, Suscripcion } from "@/types"
import { getClaseEstado } from "@/types"

// Mock data actualizado con colores corregidos
...
export function CorrectedDashboardSuscriptor() {
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="container mx-auto p-4 max-w-6xl">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-2 text-foreground">Mis Clases</h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <Card>
            <CardContent className="p-4">
              ...
              <div className="text-sm text-slate-600">Total esta semana</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4">
          ...
          <Card>
            <CardContent className="p-4">
              <div className="text-2xl font-bold text-slate-700">{stats.pendientes}</div>
          ...
          <CardContent className="space-y-6">
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                <Label className="text-base font-medium text-foreground">Alarmas por email</Label>
        ...
        {/* Lista de Clases con colores corregidos */}
                    ...
                    <CardHeader className="pb-3">
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <CardTitle className="text-lg leading-tight mb-1 text-foreground">
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center gap-2 text-sm">
                          ...
                          <span className="font-medium text-foreground">{formatearFecha(clase.fecha)}</span>
                        </div>
                        <div className="flex items-center gap-2 text-sm">
                      ...
                      {/* Información del aula con colores corregidos */}
                      <div className="space-y-2">
                        {clase.aula && clase.aula_info ? (
                          <div className={`${colors.bg} ${colors.border} rounded-lg p-3 space-y-2 border`}>
                            <div className="flex items-center gap-2">
                              <MapPin className={`h-4 w-4 ${colors.icon}`} />
                              <span className={`font-medium text-sm ${colors.text}`}>{clase.aula_info.nombre}</span>
                              <Badge variant="outline" className="text-xs">
                                {clase.aula_info.codigo}
                              </Badge>
                            </div>
                            <div className={`text-xs ${colors.text} pl-6`}>{clase.aula_info.ubicacion}</div>
                            <div className={`flex items-center gap-2 text-xs ${colors.text} pl-6`}>
                      ...
                      {clase.materia.docente && (
                        <div className="border-t pt-3">
                          <div className="flex items-center gap-2 text-sm">
                            ...
                            <div>
                              <p className="font-medium text-foreground">{clase.materia.docente.nombre}</p>
